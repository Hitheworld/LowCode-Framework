...
export const filterDate = (
  ...
  if (value && typeof value === 'string' && (m = relativeValueRe.exec(value))) {
    ...
    return m[2] === '-'
      ? from.subtract(step, timeUnitMap[m[4]] as dayjs.ManipulateType)
      : from.add(step, timeUnitMap[m[4]] as dayjs.ManipulateType);
...
export const filters: {
  ...
  toDate: (input: any, inputFormat = '') => {
    const data = dayjs(input, inputFormat);
    data.add();
 ...
 * 注册
 */
export function register(): Types.Enginer & { name: string } {
  return {
    name: 'builtin',
